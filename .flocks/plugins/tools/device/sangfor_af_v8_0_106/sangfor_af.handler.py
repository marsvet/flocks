"""
Sangfor AF (Application Firewall) v8.0.106 API Handler (latest version).

Extends v8.0.85 with:
  - Session recheck config (3.1.4.8/9)
  - Session export (3.1.4.10)
  - Block session (3.1.4.6)
  - Batch clear sessions (3.1.4.7)
  - CPU/Memory trend curves (8.x)
  - Alarm notifications: events, providers, templates, messages
  - Extended session count/summary info
  - Interface statistics (enhanced)
  - TOP N settings (3.4.2)
  - Link probe/SLA object APIs (4.2.x)

Authentication: same as v8.0.48/85 (session-based Cookie token).
"""
...
SERVICE_ID = "sangfor_af_v8_0_106"
...
def _resolve_runtime_config() -> tuple[str, int, str, str, bool]:
    ...
    username = (
        ...
        or sm.get("sangfor_af_v8_0_106_username")
        or os.getenv("AF_USERNAME")
    )
    password = (
        _resolve_ref(raw.get("password"))
        or sm.get("sangfor_af_v8_0_106_password")
        or os.getenv("AF_PASSWORD")
    )
    if not username or not password:
        raise ValueError(
            "AF API credentials not configured. "
            "Please set username and password in the sangfor_af_v8_0_106 service configuration."
        )
...
async def _login(session, base_url, username, password, verify_ssl):
    ...
    code = data.get("code")
    if code != 0:
        return None, f"AF login failed (code={code}): {data.get('message', 'Unknown')}"
...
def _af_result(action: str, payload: Any) -> ToolResult:
    metadata = {"source": "Sangfor AF", "api": action, "version": "8.0.106"}
...
async def _call(method, path, params=None, json=None, action="") -> ToolResult:
    try:
...
async def _do_login(ctx: ToolContext, **params: Any) -> ToolResult:
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
        ...
        if err:
            return ToolResult(success=False, error=err)
        _TOKEN_CACHE[base_url] = token
        return ToolResult(success=True, output={"token": token, "message": "Login successful"}, metadata={"source": "Sangfor AF", "api": "login", "version": "8.0.106"})
...
# Link probe (new in v8.0.106 objects chapter 4.2)
async def _do_get_link_probes(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    query = _pick(params, "_start", "_length", "_search", "_order", "_sortby")
    return await _call("GET", f"{API_V1}/linkprobes", params=query or None, action="get_link_probes")


async def _do_get_link_probe(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    return await _call("GET", f"{API_V1}/linkprobes/{params.get('uuid', '')}", action="get_link_probe")
...
async def _do_get_user_traffic_rank(ctx: ToolContext, **params: Any) -> ToolResult:
    ...
    return await _call("POST", f"{API_V1}/topusertraffics", params={"_method": "GET"}, json=body or {}, action="get_user_traffic_rank")
...
async def _do_get_vsys_session_summary(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    query = _pick(params, "vsys")
    return await _call("GET", f"{API_V1}/vsyssessionsummary", params=query or None, action="get_vsys_session_summary")


# Session management actions (new in v8.0.106)
async def _do_clear_session(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    body = _pick(params, "srcIP", "dstIP", "srcPort", "dstPort", "protocol", "vsys")
    return await _call("DELETE", f"{API_V1}/sessions", json={"obj": body}, action="clear_session")


async def _do_block_session(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    body = _pick(params, "srcIP", "dstIP", "srcPort", "dstPort", "protocol", "vsys")
    return await _call("POST", f"{API_V1}/sessions/block", json={"obj": body}, action="block_session")


async def _do_batch_clear_sessions(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    items = params.get("items", [])
    return await _call("POST", f"{API_BATCH}/sessions", params={"_method": "DELETE"}, json=items, action="batch_clear_sessions")


async def _do_get_session_recheck(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    return await _call("GET", f"{API_V1}/sessionrecheck", action="get_session_recheck")


async def _do_set_session_recheck(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    body = _pick(params, "enable", "interval")
    return await _call("PUT", f"{API_V1}/sessionrecheck", json={"obj": body}, action="set_session_recheck")
...
# ── Alarm/notification actions (new in v8.0.106, section 3.4.3) ──────────────

async def _do_get_alarm_events_config(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    return await _call("GET", f"{API_V1}/alarm/events", action="get_alarm_events_config")


async def _do_get_alarm_notifications(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    return await _call("GET", f"{API_V1}/alarm/notifications", action="get_alarm_notifications")


async def _do_get_alarm_messages(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    query = _pick(params, "_start", "_length", "_search", "_order", "_sortby")
    return await _call("GET", f"{API_V1}/alarm/messages", params=query or None, action="get_alarm_messages")


async def _do_get_topn_config(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    return await _call("GET", f"{API_V1}/topnconfig", action="get_topn_config")
...
async def _do_get_memory_trend(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    query = _pick(params, "minutes")
    return await _call("GET", f"{API_V1}/memoryusagetrend", params=query or None, action="get_memory_trend")
...
async def _do_get_cpu_trend(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    query = _pick(params, "minutes")
    return await _call("GET", f"{API_V1}/cpuusagetrend", params=query or None, action="get_cpu_trend")
...
async def _do_get_interface_throughput(ctx: ToolContext, **params: Any) -> ToolResult:
    del ctx
    query = _pick(params, "ifname", "minutes")
    return await _call("GET", f"{API_V1}/interfacethroughput", params=query or None, action="get_interface_throughput")
...
_ACTION_MAP: dict[str, Callable] = {
    ...
    "get_link_probes": _do_get_link_probes,
    "get_link_probe": _do_get_link_probe,
    # Monitoring
    ...
    "get_vsys_session_summary": _do_get_vsys_session_summary,
    "get_sessions": _do_get_sessions,
    "get_monitor_ips": _do_get_monitor_ips,
    # Session management
    "clear_session": _do_clear_session,
    "block_session": _do_block_session,
    "batch_clear_sessions": _do_batch_clear_sessions,
    "get_session_recheck": _do_get_session_recheck,
    "set_session_recheck": _do_set_session_recheck,
    # Statistics
    ...
    # Alarm/notification
    "get_alarm_events_config": _do_get_alarm_events_config,
...
GROUP_ACTIONS: dict[str, set[str]] = {
    ...
    "objects": {
        "get_ipgroups", "get_ipgroup", "create_ipgroup", "update_ipgroup", "delete_ipgroup",
        "get_services", "get_service",
        "get_link_probes", "get_link_probe",
    },
    "monitor": {
        ...
        "get_vsys_session_summary", "get_sessions", "get_monitor_ips",
        "clear_session", "block_session", "batch_clear_sessions",
        "get_session_recheck", "set_session_recheck",
        "get_packet_drop_stats", "clear_packet_drop_stats",
        "get_mbuf_stats", "get_hash_table_stats",
    },
    "alarm": {
        "get_alarm_events_config", "get_alarm_notifications",
        "get_alarm_messages", "get_topn_config",
    },
    "ops": {
    ...
    "status": {
        "get_memory_usage", "get_memory_trend", "get_cpu_usage", "get_cpu_trend",
        "get_disk_usage", "get_system_version", "get_interface_status",
        "get_interface_throughput", "get_runtime_status", "get_current_time",
    },
...
async def alarm(ctx: ToolContext, action: str, **params: Any) -> ToolResult:
    return await _dispatch_group(ctx, "alarm", action, **params)
