...
DEFAULT_API_PREFIX = ""
# OAEP 哈希仅用于*非登录*的敏感字段加密（改密、删用户、删审计日志等），
# 登录阶段已切换为明文密码。保留该配置项是为了与新版 onesig 共享同一份
# encrypt_fields 写操作链路。
...
def _resolve_runtime_config() -> OneSIGRuntimeConfig:
    ...
    # 升级口令）有效；登录阶段已改为明文，详见 ``OneSIGSession.login``。
    oaep_hash = (
...
class OneSIGSession:
    ...
    async def login(
        ...
        *,
        ...
        """Run the *plaintext-password* login flow for OneSIG v2.5 older devices.

        Sequence: ``GET /v3/captcha`` → ``POST /v3/login`` (with plaintext
        ``password``) → optional ``POST /v3/login/totp``. The
        ``GET /v3/pubkey`` + RSA-OAEP encryption step from the standard
        ``onesig`` plugin is intentionally omitted here — older firmware
        rejects RSA ciphertext on the login endpoint and only accepts the
        raw password.

        Returns the final account info on success. Raises ValueError on
        ...
        async with self._login_lock:
            ...
            # ---------------------------------------------------------
            # OneSIG v2.5 老版本走 *明文* 密码登录链路 ——
            #
            # 与新版 ``onesig`` 插件最显著的区别：跳过 ``GET /v3/pubkey`` +
            # RSA-OAEP 加密这一对步骤，直接把 ``self.config.password``
            # 的明文塞进 ``payload["password"]`` 发给 ``POST /v3/login``。
            #
            # 设计取舍：
            #   * 老版本设备早期部署（v2.5 之前的中间发布或定制版）只接受
            #     明文密码登录，下发 RSA 密文反而会被解析成乱码导致
            #     ``responseCode=1009`` / ``1017``。
            #   * 业务路径上的*敏感写字段*（改密、删用户、删审计、接口启停、
            #     设备升级口令等）仍走 ``encrypt_with_pubkey`` /
            #     ``_encrypt_body_fields``，复用同一份 OAEP 实现。换言之：
            #     **只有登录阶段是明文**，写操作字段加密保持与新版一致。
            #   * 该插件应仅用于内网受控网络中的老固件，避免明文凭证暴露
            #     于未加密信道；HTTPS + 启用 ``verify_ssl`` 是强烈建议项。
            # ---------------------------------------------------------
