...
- 时间字段硬规则：凡是本次 API 调用涉及 `time_from` / `time_to` 等时间字段，必须先在 bash 中执行 `uv run python` 动态计算，再调用对应 tool；禁止手动估算、禁止硬编码、禁止把“今天”“最近 7 天”“最近 1 小时”等自然语言时间直接脑补成数字。
- TDP 的时间入参默认按 Unix 秒级时间戳处理；无论是日志、事件、看板还是风险类查询，只要要传时间范围，都必须先用 `uv run python` 算出准确边界后再传参。
