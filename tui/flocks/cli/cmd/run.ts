...
import { createFlocksClient, type FlocksClient } from "@flocks-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run flocks with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running flocks server (e.g., http://localhost:8000)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: FlocksClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createFlocksClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.FLOCKS_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createFlocksClient({ baseUrl: "http://flocks.internal", fetch: fetchFn })
