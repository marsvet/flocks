...
import type { QuestionAnswer, QuestionRequest } from "@flocks-ai/sdk/v2"
...
export function QuestionPrompt(props: { request: QuestionRequest }) {
  ...
  function isEnterKey(name: string) {
  ...
  async function submit(trigger: string, providedAnswers?: QuestionAnswer[]) {
    ...
    log.info("question.submit.triggered", {
      trigger,
      requestID: id,
      tab: store.tab,
      confirm: confirm(),
    ...
    try {
      ...
      log.info("question.submit.succeeded", {
        trigger,
        requestID: id,
      })
    } catch (error) {
      log.error("question.submit.failed", {
        trigger,
        requestID: id,
        error,
      })
  ...
  async function reject(trigger: string) {
    const id = requestID()
    if (!id) {
      log.warn("question.reject.skipped.missing_request", {
        trigger,
        tab: store.tab,
        confirm: confirm(),
        editing: store.editing,
    ...
    log.info("question.reject.triggered", {
      trigger,
      requestID: id,
      tab: store.tab,
      confirm: confirm(),
    ...
    try {
      ...
      log.info("question.reject.succeeded", {
        trigger,
        requestID: id,
      })
    } catch (error) {
      log.error("question.reject.failed", {
        trigger,
        requestID: id,
        error,
      })
  ...
  useKeyboard((evt) => {
    ...
    if (confirm()) {
      if (isEnterKey(evt.name)) {
        evt.preventDefault()
        void submit("enter_on_confirm")
      }
      if (evt.name === "escape" || keybind.match("app_exit", evt)) {
        evt.preventDefault()
        void reject("escape_on_confirm")
      }
    } else {
      const opts = options()
      ...
      if (evt.name === "escape" || keybind.match("app_exit", evt)) {
        evt.preventDefault()
        void reject("escape_on_question")
      }
    }
  })
