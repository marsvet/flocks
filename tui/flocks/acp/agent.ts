...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: FlocksClient }) {
    return {
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Flocks stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    if (flocksProvider) {
      if (flocksProvider.models["big-pickle"]) {
        return { providerID: "opencode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(flocksProvider.models))
      if (best) {
        return {
